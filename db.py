...
class DB:
    ...
    def get_last_article(self):
        """Получаем code последней новости"""
        with self.connection:
            return self.cursor.execute("SELECT `last_article` FROM `article` WHERE `id` = 1").fetchall()

    def update_last_article(self, last_article):
        """обновление code последней новости"""
        with self.connection:
            return self.cursor.execute("UPDATE `article` SET `last_article` = ? WHERE `id` = 1", (last_article,)).fetchall()
